...
export default function TemplateCustomize({ template, userTemplate }: PageProps) {
    ...
    const handleSaveDraft = (e: React.FormEvent) => {
        ...
        put(`/reseller/user-templates/${userTemplate.id}/save-draft`);
    };
    ...
    const handleConfirmPurchase = () => {
        ...
        put(`/reseller/user-templates/${userTemplate.id}/save-draft`, {
            onSuccess: async () => {
                try {
                    const response = await fetch(`/reseller/user-templates/${userTemplate.id}/purchase`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (!response.ok) {
                        let errorMessage = 'Failed to purchase template.';
                        try {
                ...
                <DialogContent className="max-w-sm bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-bold flex items-center gap-2">
                            Wallet Purchase Confirmation <Sparkles className="size-5 text-amber-500" />
                        </DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="py-4 flex flex-col gap-4 text-sm text-neutral-600 dark:text-neutral-400">
                        ...
                        <p className="text-xs text-neutral-400 leading-normal">
                            *Clicking confirm will deduct the amount from your reseller wallet and unlock the template for download/sharing.
                        </p>
                    </div>
                    ...
                    <DialogFooter className="gap-2">
                        ...
                        <Button
                            ...
                            className="bg-indigo-700 hover:bg-indigo-700 text-white font-bold rounded-xl px-5"
                        >
                            {isCheckingOut ? 'Processing...' : 'Pay from Wallet'}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
